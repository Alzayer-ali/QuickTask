...
object SmartTimeParser {
     ...
     */
    private fun calculateNextWeekday(targetCalendarDay: Int, refCal: Calendar = Calendar.getInstance()): Long {
        val cal = (refCal.clone() as Calendar).apply {
    ...
    /**
     * Calculates normalized start-of-day millis offset by a given number of days from referenceCal.
     */
    private fun getDayStartMillis(refCal: Calendar, dayOffset: Int = 0): Long {
        val cal = (refCal.clone() as Calendar).apply {
     ...
     * Fully operates in 24-hour time format with complete support for AM/PM indicators and smart upcoming time resolution.
     */
    fun parse(rawText: String, referenceCal: Calendar = Calendar.getInstance()): ParsedTaskResult {
        if (rawText.isBlank()) {
        ...
        if (parsedRecurrence != RecurrenceType.NONE && parsedDueDate == null) {
            parsedDueDate = getDayStartMillis(referenceCal, 0)
        }
        ...
        if (parsedDueDate == null) {
            ...
            if (matchWeekday != null) {
                ...
                if (targetDayOfWeek != null) {
                    parsedDueDate = calculateNextWeekday(targetDayOfWeek, referenceCal)
                    hasExplicitDate = true
        ...
        if (inTwoHoursRegex.containsMatchIn(workingText)) {
            val cal = (referenceCal.clone() as Calendar).apply { add(Calendar.HOUR_OF_DAY, 2) }
            startHour = cal.get(Calendar.HOUR_OF_DAY)
            startMinute = cal.get(Calendar.MINUTE)
            parsedDueDate = getDayStartMillis(cal, 0)
            hasExplicitDate = true
            workingText = workingText.replace(inTwoHoursRegex, " ")
        } else if (inOneHourRegex.containsMatchIn(workingText)) {
            val cal = (referenceCal.clone() as Calendar).apply { add(Calendar.HOUR_OF_DAY, 1) }
            startHour = cal.get(Calendar.HOUR_OF_DAY)
            startMinute = cal.get(Calendar.MINUTE)
            parsedDueDate = getDayStartMillis(cal, 0)
            hasExplicitDate = true
            ...
            if (matchXHours != null) {
                ...
                val cal = (referenceCal.clone() as Calendar).apply { add(Calendar.HOUR_OF_DAY, hours) }
                startHour = cal.get(Calendar.HOUR_OF_DAY)
                startMinute = cal.get(Calendar.MINUTE)
                parsedDueDate = getDayStartMillis(cal, 0)
                hasExplicitDate = true
                ...
                if (matchXMins != null) {
                    ...
                    val cal = (referenceCal.clone() as Calendar).apply { add(Calendar.MINUTE, mins) }
                    startHour = cal.get(Calendar.HOUR_OF_DAY)
                    startMinute = cal.get(Calendar.MINUTE)
                    parsedDueDate = getDayStartMillis(cal, 0)
                    hasExplicitDate = true
        ...
        if (startHour == null) {
            val rangeRegex = Regex(
                """(?i)(?:(من\s+(?:الساعة\s+)?|from\s+)|(?:الساعة\s+))?(\d{1,2})(?::(\d{2}))?\s*($periodWords)?\s*(?:إلى|الى|حتى|to|-)\s*(?:(?:الساعة\s+)?)?(\d{1,2})(?::(\d{2}))?\s*($periodWords)?"""
            )
            val rangeMatch = rangeRegex.find(workingText)
            if (rangeMatch != null) {
                val hasExplicitRangePrefix = rangeMatch.groupValues[1].isNotEmpty()
                val sH = rangeMatch.groupValues[2].toIntOrNull()
                val sMStr = rangeMatch.groupValues[3]
                val sM = sMStr.ifEmpty { "0" }.toIntOrNull() ?: 0
                val sPeriod = rangeMatch.groupValues[4].ifEmpty { null }

                val eH = rangeMatch.groupValues[5].toIntOrNull()
                val eMStr = rangeMatch.groupValues[6]
                val eM = eMStr.ifEmpty { "0" }.toIntOrNull() ?: 0
                val ePeriod = rangeMatch.groupValues[7].ifEmpty { null }

                val isHyphen = rangeMatch.value.contains("-")
                val isLegitimateRange = hasExplicitRangePrefix ||
                        sPeriod != null ||
                        ePeriod != null ||
                        !isHyphen || // uses 'إلى', 'الى', 'حتى', 'to'
                        (sMStr.isNotEmpty() && eMStr.isNotEmpty()) // formatted as 15:00 - 17:00

                if (isLegitimateRange && sH != null && eH != null && sH in 0..23 && eH in 0..23) {
                    ...
                    if (convertedStart in 0..23 && convertedEnd in 0..23) {
                        ...
                        if (!hasExplicitDate) {
                            val nowHour = referenceCal.get(Calendar.HOUR_OF_DAY)
                            val nowMinute = referenceCal.get(Calendar.MINUTE)
                            val nowTotal = nowHour * 60 + nowMinute

                            if (sPeriod == null && ePeriod == null && startHour in 1..11 && endHour in 1..11) {
                                val endAmTotal = endHour * 60 + (endMinute ?: 0)
                                val endPmTotal = (endHour + 12) * 60 + (endMinute ?: 0)
                                if (endAmTotal <= nowTotal && endPmTotal > nowTotal) {
                                    startHour = (startHour + 12) % 24
                                    endHour = (endHour + 12) % 24
                                    parsedDueDate = getDayStartMillis(referenceCal, 0)
                                } else if (endPmTotal <= nowTotal) {
                                    parsedDueDate = getDayStartMillis(referenceCal, 1)
                                } else {
                                    parsedDueDate = getDayStartMillis(referenceCal, 0)
                                }
                            } else {
                                val endTotal = endHour * 60 + (endMinute ?: 0)
                            ...
                            parsedDueDate = parsedDueDate ?: getDayStartMillis(referenceCal, 0)
                        }
            ...
            if (matchPeriod != null) {
                ...
                if (rawH != null) {
                    ...
                    if (convertedH in 0..23) {
                        ...
                        // Requirement 2: Do not set end time when only start time is given
                        endHour = null
                        endMinute = null
                        ...
                        if (!hasExplicitDate) {
                            ...
                            val nowMinute = referenceCal.get(Calendar.MINUTE)
                            val nowTotal = nowHour * 60 + nowMinute
                            val taskTotal = convertedH * 60 + (startMinute ?: 0)

                            if (taskTotal <= nowTotal) {
                                parsedDueDate = getDayStartMillis(referenceCal, 1)
                            } else {
                                parsedDueDate = getDayStartMillis(referenceCal, 0)
                            }
        ...
        // 8. Single 24-Hour or Ambiguous 12-Hour Time Detection (e.g., "الساعة 5", "الساعة 15:30", "at 14:00", "16:45")
        if (startHour == null) {
            var rawH: Int? = null
            var rawM = 0
            ...
            if (singlePrefixMatch != null) {
                rawH = singlePrefixMatch.groupValues[1].toIntOrNull()
                rawM = singlePrefixMatch.groupValues[2].ifEmpty { "0" }.toIntOrNull() ?: 0
                matchedSubstring = singlePrefixMatch.value
            } else {
                // Standalone time formatted as HH:mm (e.g. 15:30, 08:00, 13:00, 5:00)
                val standaloneTimeRegex = Regex("""\b([01]?\d|2[0-3]):([0-5]\d)\b""")
                val standaloneMatch = standaloneTimeRegex.find(workingText)
                if (standaloneMatch != null) {
                    rawH = standaloneMatch.groupValues[1].toIntOrNull()
                    rawM = standaloneMatch.groupValues[2].toIntOrNull() ?: 0
                    matchedSubstring = standaloneMatch.value
                }
            }

            if (rawH != null && rawH in 0..23 && matchedSubstring != null) {
                val clampedMinute = rawM.coerceIn(0, 59)
                // Requirement 2: Ensure endHour and endMinute are explicitly null
                endHour = null
                endMinute = null
                ...
                if (parsedRecurrence != RecurrenceType.NONE) {
                    // For recurring routines (e.g. daily 7:00), preserve exact hour as specified
                    startHour = rawH
                    startMinute = clampedMinute
                    parsedDueDate = parsedDueDate ?: getDayStartMillis(referenceCal, 0)
                } else if (!hasExplicitDate) {
                    // SMART TIME RESOLUTION: No explicit date was given!
                    if (rawH in 1..11) {
                        val amTotal = rawH * 60 + clampedMinute
                        val pmTotal = (rawH + 12) * 60 + clampedMinute

                        when {
                            amTotal > nowTotal -> {
                                // Morning occurrence is upcoming today
                                startHour = rawH
                                startMinute = clampedMinute
                                parsedDueDate = getDayStartMillis(referenceCal, 0)
                            }
                            pmTotal > nowTotal -> {
                                // Evening occurrence is upcoming today (e.g. now 13:00, input "الساعة 5" -> 17:00 today)
                                startHour = rawH + 12
                                startMinute = clampedMinute
                                parsedDueDate = getDayStartMillis(referenceCal, 0)
                            }
                            else -> {
                                // Both passed today, so it's morning next day (e.g. now 19:00, input "الساعة 5" -> 05:00 tomorrow)
                                startHour = rawH
                                startMinute = clampedMinute
                                parsedDueDate = getDayStartMillis(referenceCal, 1)
                            }
                        ...
                        val noonTotal = 12 * 60 + clampedMinute
                        when {
                            noonTotal > nowTotal -> {
                                startHour = 12
                                startMinute = clampedMinute
                    ...
                    } else if (rawH in 13..23) {
                        val taskTotal = rawH * 60 + clampedMinute
                        if (taskTotal <= nowTotal) {
                            startHour = rawH
                            startMinute = clampedMinute
                            parsedDueDate = getDayStartMillis(referenceCal, 1)
                    ...
                    } else { // rawH == 0 (midnight)
                        if (clampedMinute <= nowMinute && nowHour == 0) {
                            startHour = 0
                            startMinute = clampedMinute
                            parsedDueDate = getDayStartMillis(referenceCal, 1)
                        } else {
                    ...
                    // Date was explicitly specified (e.g. "غدا الساعة 5", "اليوم الساعة 5")
                    val isExplicitlyToday = parsedDueDate == getDayStartMillis(referenceCal, 0)
                    if (isExplicitlyToday && rawH in 1..11) {
                        val amTotal = rawH * 60 + clampedMinute
                        val pmTotal = (rawH + 12) * 60 + clampedMinute
                        startHour = if (amTotal > nowTotal) rawH else rawH + 12
                    } else {
                        startHour = rawH
                    }
                    startMinute = clampedMinute
                ...
                workingText = workingText.replace(matchedSubstring, " ")
            }
        }
